...
public final class OAuth2AuthorizationEndpointFilter extends OncePerRequestFilter {
	...
	public OAuth2AuthorizationEndpointFilter(AuthenticationManager authenticationManager) {
		this(authenticationManager, ApplicationConstant.DEFAULT_AUTHORIZATION_ENDPOINT_URI);
	}
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			log.error("Request QueryParams: [" + request.getQueryString() + "].");
			request.getParameterMap().forEach((key, value) -> log.error("Request ParamMap Key:[" + key + "]:[" + StringUtils.arrayToCommaDelimitedString(value) +"]"));
