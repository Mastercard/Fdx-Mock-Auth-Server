...
public final class OAuth2AuthorizationCodeAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        // ----- Refresh token -----
        //Logic to Determine if request was ONE_TIME_CONSENT (ie PAR_CLAIMS_SHARING_DURATION == 0)
        OAuth2RefreshToken refreshToken = null;
        if (registeredClient.getAuthorizationGrantTypes().contains(AuthorizationGrantType.REFRESH_TOKEN) &&
                // Do not issue refresh token to public client
                !clientPrincipal.getClientAuthenticationMethod().equals(ClientAuthenticationMethod.NONE) &&
                !isOneTimeConsentRequest(authorizationRequest)) {
        ...
        //FDX 6.0: Successful token response will include the grant_id parameter containing ConsentId in addition to token response contents
        String consent_id = ((ClaimAccessor) generatedAccessToken).getClaimAsString(ApplicationConstant.FDX_CONSENT_ID);
        if (StringUtils.hasText(consent_id)) {
            additionalParameters.put(ApplicationConstant.GRANT_ID, consent_id);
        }
