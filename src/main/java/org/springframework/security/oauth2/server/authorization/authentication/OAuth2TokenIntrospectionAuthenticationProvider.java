...
public final class OAuth2TokenIntrospectionAuthenticationProvider implements AuthenticationProvider {
    ...
    //Updated to be more generic to allow JwtAuthenticationToken for Authentication
    private Authentication getAuthenticatedClientElseThrowInvalidClient(Authentication authentication) {
        Authentication clientPrincipal = null;
        if (Authentication.class.isAssignableFrom(authentication.getPrincipal().getClass())) {
            clientPrincipal = (Authentication) authentication.getPrincipal();
        }
